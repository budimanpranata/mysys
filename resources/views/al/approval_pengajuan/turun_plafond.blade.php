...
@section('content-header')
        ...
        <div class="col-md-12">
            <div class="card card-primary">
                <div class="card-body">
                    <div class="form-group row">
                        ...
                        <div class="col-sm-6">
                            <select class="form-control" id="jenis_pengajuan">
                                ...
                                <option value="turun_plafond" selected>Turun Plafond</option>
                    ...
                    {{-- FORM HARGA BARU --}}
                    <div class="form-group row" id="form-harga_baru">
                        <label class="col-sm-2 col-form-label">Harga Baru</label>
                        <div class="col-sm-6">
                            <div class="input-group">
                                <input type="text" class="form-control" id="harga_baru">
                            </div>
                        </div>
                    </div>
    ...
    let cif = $('#cif').val();
    let hargaBaru = $('#harga_baru').val();

    if (!cif) {
        Swal.fire('Peringatan', 'Silakan pilih CIF', 'warning');
        return;
    }

    if (!hargaBaru) {
        Swal.fire('Peringatan', 'Harga baru wajib diisi', 'warning');
        return;
    }

    Swal.fire({
        title: 'Yakin?',
        text: 'Plafond lama akan diganti dengan harga baru',
        icon: 'question',
        ...
        if (result.isConfirmed) {
            $.ajax({
                url: "{{ route('turun.plafond.proses') }}",
                type: "POST",
                data: {
                    cif: cif,
